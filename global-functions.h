...
extern struct s_client * create_client(IP_STRUCT(*));
...
extern void cs_add_violation_by_ip(IP_STRUCT(ip), int32_t port, char *info);
...
extern char *cs_inet_ntoa(IP_STRUCT(addr));
extern void cs_inet_addr(char *txt, IP_STRUCT(* out));
...
extern int32_t check_ip(struct s_ip *ip, IP_STRUCT(n));
